...
__MODULE__ = "Convert"
__HELP__ = """
<blockquote><b>✘ Bantuan Untuk Convert</b></blockquote>

<blockquote><b>๏ Perintah: {0}toanime [Reply Photo/Sticker/Gift]
◉ Penjelasan: Untuk Merubah Photo/Sticker/Gift Menjadi Anime</b></blockquote>
<blockquote><b>๏ Perintah: {0}toimg [Reply Sticker/Gift]
◉ Penjelasan: Untuk Merubah Sticker/Gift Menjadi Photo</b></blockquote>
<blockquote><b>๏ Perintah: {0}tosticker [Reply Photo]
◉ Penjelasan: Untuk Merubah Foto Menjadi Sticker</b></blockquote>
<blockquote><b>๏ Perintah: {0}togif [Reply Sticker]
◉ Penjelasan: Untuk Merubah Sticker Menjadi Gift</b></blockquote>
<blockquote><b>๏ Perintah: {0}toaudio [Reply Video]
◉ Penjelasan: Untuk Merubah Video Menjadi Audio</b></blockquote>
<blockquote><b>๏ Perintah: {0}efek [Efek Code - Reply Voice Note]
◉ Penjelasan: Untuk Merubah Suara Voice Note</b></blockquote>
<blockquote><b>๏ Perintah: {0}list_efek
◉ Penjelasan: Untuk Melihat Daftar Efek</b></blockquote>
<blockquote><b>๏ Perintah: {0}colong [Reply Media Timer]
◉ Penjelasan: Buat Ngambil Pap Tt Anjeng Masuk Nya Ke Pesan Tersimpan Kontol</b></blockquote>
"""
...
async def convert_anime(client, message):
    ...
    if message.reply_to_message:
        if len(message.command) < 2:
            if message.reply_to_message.photo:
                ...
                return await Tm.edit("<b>Mohon Balas Ke</b> <code>Photo/Sticker/Git</code>")
        else:
        ...
        if len(message.command) < 2:
            return await Tm.edit("Balas Ke Foto Dan Saya Akan Merubah Foto Anda Menjadi Anime")
        else:
            try:
                file = "foto"
    ...
    async for anime in client.search_messages("@qq_neural_anime_bot", filter=MessagesFilter.PHOTO):
        anime_photo.append(InputMediaPhoto(anime.photo.file_id, caption=f"<b>Powered By: {bot.me.mention}</b>"))
    if anime_photo:
...
async def convert_sticker(client, message):
    try:
        if not message.reply_to_message or not message.reply_to_message.photo:
            return await message.reply_text("Reply Ke Foto Yang Mau Di Jadikan Sticker")
...
async def convert_gif(client, message):
    ...
    if not message.reply_to_message.sticker:
        return await TM.edit("<b>Balas Ke Sticker...</b>")
...
async def convert_audio(client, message):
    ...
    if replied.media == MessageMediaType.VIDEO:
        ...
        return await Tm.edit("<b>Balas Ke Video Kontol</b>")
...
async def convert_efek(client, message):
    ...
    if reply and list_efek:
        if arga in list_efek:
            Tm = await message.reply(f"Merubah Suara Menjadi {args}")
            ...
            await message.reply("Silahkan Ketil `{}list_efek` Untuk Melihat Daftar Efek".format(next((p) for p in prefix)))
    else:
        await message.reply("Silahkan Ketik `{}list_efek` Untuk Melihat Daftar Efek".format(next((p) for p in prefix)))
...
async def colong_cmn(client, message):
    ...
    if dia.document:
        ...
        return await message.reply("Omaaaakk Ada Kesalahan Tod")
