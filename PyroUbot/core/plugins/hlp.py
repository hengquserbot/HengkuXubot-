...
async def help_cmd(client, message):
    if not get_arg(message):
        ...
        module = gen_font(get_arg(message), font["normal"])
        if get_arg(message) in HELP_COMMANDS:
            ...
            await message.reply(f"<b>❌ Tidak Dapat Menemukan Modul Dengan Nama<code>{module}</code></b>")


async def menu_inline(client, inline_query):
    prefix = await ubot.get_prefix(inline_query.from_user.id)
    msg = f"<b>Help Inline Modules </b>\n<b>User : <code><a href=tg://user?id={inline_query.from_user.id}>{inline_query.from_user.first_name} {inline_query.from_user.last_name or ''}</a></code></b>\n<b>Prefix:</b> {' '.join(prefix)}"
...
async def menu_callback(client, callback_query):
    ...
    top_text = f"<b>Help Inline Modules </b>\n<b>User : <code><a href=tg://user?id={callback_query.from_user.id}>{callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}</a></code></b>\n<b>Prefixes:</b> {' '.join(prefix)}"
    if mod_match:
